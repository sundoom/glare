...
====================
*/
VOID Texture::Load(U32 level, U32 width, U32 height, U32 bits, U32 format, U32 type, const U8* pixels)
{
	// store the info
	mWidth = width;
	mHeight = height;
  ...
  BindTexture(mTarget, mObject);
    ...
    if(height == 0) height = 1;
      ...
      pixel_size = (((width+3)/4) * ((height+3)/4) * 8);
			glCompressedTexImage2D(GL_TEXTURE_2D, i, format, width, height, 0, pixel_size, pixels);
			break;
		case PF_RGBA_DXT1:
      pixel_size = (((width+3)/4) * ((height+3)/4) * 8);
			glCompressedTexImage2D(GL_TEXTURE_2D, i, format, width, height, 0, pixel_size, pixels);
...
====================
*/
VOID Texture::Update(U32 level, I32 x, I32 y, U32 width, U32 height, U32 bits, U32 format, U32 type, const U8* pixels)
{
	CHECK(mObject);
    ...
    if(height == 0) height = 1;
		...
		case PF_RGB_DXT1:
      pixel_size = (((width+3)/4) * ((height+3)/4) * 8);
			glCompressedTexSubImage2D(GL_TEXTURE_2D, i, x, y, width, height, format, pixel_size, pixels);
			break;
		case PF_RGBA_DXT1:
      pixel_size = (((width+3)/4) * ((height+3)/4) * 8);
			glCompressedTexSubImage2D(GL_TEXTURE_2D, i, x, y, width, height, format, pixel_size, pixels);
